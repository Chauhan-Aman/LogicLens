...
inline void __ll_set_var(const std::string& name, const T& value) {
    std::cout << "{\"type\": \"VARIABLE_UPDATE\", \"payload\": {\"name\": \"" << name << "\", \"value\": " << __ll_to_json(value) << "}}\n";
}

template <typename T>
inline void __ll_array_access(const std::string& name, const T& index) {
    std::cout << "{\"type\": \"ARRAY_ACCESS\", \"payload\": {\"name\": \"" << name << "\", \"index\": " << index << "}}\n";
}

template <typename T, typename U>
inline void __ll_array_write(const std::string& name, const T& index, const U& value) {
    std::cout << "{\"type\": \"ARRAY_WRITE\", \"payload\": {\"name\": \"" << name << "\", \"index\": " << index << ", \"value\": " << __ll_to_json(value) << "}}\n";
}

template <typename K, typename V>
inline void __ll_map_insert(const std::string& name, const K& key, const V& value) {
    std::cout << "{\"type\": \"MAP_INSERT\", \"payload\": {\"name\": \"" << name << "\", \"key\": " << __ll_to_json(key) << ", \"value\": " << __ll_to_json(value) << "}}\n";
}

template <typename K>
inline void __ll_map_lookup(const std::string& name, const K& key) {
    std::cout << "{\"type\": \"MAP_LOOKUP\", \"payload\": {\"name\": \"" << name << "\", \"key\": " << __ll_to_json(key) << "}}\n";
}

inline void __ll_note(const std::string& message) {
    std::cout << "{\"type\": \"ANNOTATION\", \"payload\": {\"message\": \"" << __ll_escape(message) << "\"}}\n";
}
